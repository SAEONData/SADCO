 ...
 */
public class ExtractMRNPhysNutDataP { //extends CompoundItem {
    ...
    /**
     * Main constructor ExtractMRNPhysNutDataP
     * @param   args   String array of arguments as from form or URL
     */
    public ExtractMRNPhysNutDataP(String args[])  {
        if (test) System.out.println("start = " + new java.util.Date());

        // Get the arguments
        getArgParms(args);
        if (test) System.out.println("surveyId = " + surveyId);
...
//        String userid = sessions[0].getUserid();
        ...
        if ("0".equals(userType)) {  // from inventory                  //ub03
            ...
            if (!dbg) {
                queue = new Queue(userid, thisClass +" "+fileNameIn);   //ub02
                queue.enQueue();                                        //ub02
...
//            updateLogUsers(userid, ec.timeDiff(date2, date0));
    ...
    } // ExtractMRNPhysNutDataP(String args[])
    ...
    public static void main(String args[]) {
        ...
        try {
            ExtractMRNPhysNutDataP local =
                new ExtractMRNPhysNutDataP(args);
        } catch(Exception e) {
            common.edmCommonPC.processErrorStatic(
                e, "ExtractMRNPhysNutDataP", "Constructor", "");
        } // try-catch
