...
//    boolean fatalErrors = false;
    ...
    String submitScientist = "Marcel van den Bergh";        // ub03
    ...
    void reInitVars() {
        ...
        chiefScientist = "";                             // ub03
        countryName = "";                                // ub03
        submitScientist = "Marcel van den Bergh";        // ub03
...
//        } // if (!institute.equals(station.getStationId().substring(0,3)))
    ...
    void format10(String line,  int lineCount) {
        ...
        //02  a12 stnid   station id: composed as for format 03   watphy
        //03  f7.2    pah µgm / litre watchem2
        //04  f7.3    rubidium    µgm / litre watchem2
        //05  f7.3    csaesium    µgm / litre watchem2
        //06  f7.2    hydrocarbons    µgm / litre watchem2
        ...
        if (replaceData) {
            ...
            if (subdesCount[index] == 0) {
                ...
                if (dataType == CURRENTS) {
                    ...
                    currentsCount -= currents.getRecCnt(where2);
                    currents.del(where2);
                    ...
                    for (int i = 0; i < delSedphy.length; i++) {
                        ...
                        // update counts
                        sedphyCount      -= 1;
                        sedchem1Count    -= sedchem1.getRecCnt(where3);
                        sedchem2Count    -= sedchem2.getRecCnt(where3);
                        sedpol1Count     -= sedpol1.getRecCnt(where3);
                        sedpol2Count     -= sedpol2.getRecCnt(where3);
                    ...
                    for (int i = 0; i < delWatphy.length; i++) {
                        ...
                        // update counts
                        watphyCount      -= 1;
                        watchem1Count    -= watchem1.getRecCnt(where3);
                        watchem2Count    -= watchem2.getRecCnt(where3);
                        watchlCount      -= watchl.getRecCnt(where3);
                        watnutCount      -= watnut.getRecCnt(where3);
                        watpol1Count     -= watpol1.getRecCnt(where3);
                        watpol2Count     -= watpol2.getRecCnt(where3);
