...
/**
 * This class manages the EDM_CLIENT table with the help of the DbAccessC
 ...
 */
public class EdmClient extends Tables {
    ...
    /** The table name */
    public static final String TABLE = "EDM_CLIENT";
    /** The code field name */
    ...
    public EdmClient() {
        clearVars();
        if (dbg) System.out.println ("<br>in EdmClient constructor 1"); // debug
    } // EdmClient constructor

    /**
     * Instantiate a EdmClient object and initialize the instance variables.
    ...
    public EdmClient(
        ...
        this();
        setCode (code);
        if (dbg) System.out.println ("<br>in EdmClient constructor 2"); // debug
    } // EdmClient constructor
    ...
    public EdmClient(
        ...
        setCode (code);
        setName (name);
        if (dbg) System.out.println ("<br>in EdmClient constructor 3"); // debug
    } // EdmClient constructor
     ...
     * are returned in an array of EdmClient. e.g.<pre>
     * EdmClient edmClient = new EdmClient(<val1>);
     * EdmClient edmClientArray[] = edmClient.get();</pre>
     * will get the EdmClient record where code = <val1>.
     * @return Array of EdmClient (EdmClient[])
     */
    public EdmClient[] get() {
     ...
     * e.g.<pre>
     * EdmClient edmClientArray[] =
     *     new EdmClient().get(EdmClient.CODE+"=<val1>");</pre>
     * will get the EdmClient record where code = <val1>.
     * @param  where  The where clause for the select statement. (String)
     *     If where == "" / null, it is ignored.
     * @return Array of EdmClient (EdmClient[])
     */
    public EdmClient[] get (String where) {
     ...
     * EdmClient edmClientArray[] =
     *     new EdmClient().get("1=1",edmClient.CODE);</pre>
     * will get the all the EdmClient records, and order them by code.
     * @param  where  The where clause for the select statement. (String)
     *     If where == "" / null, it is ignored.
     ...
     * @return Array of EdmClient (EdmClient[])
     */
    public EdmClient[] get (String where, String order) {
     ...
     * String columns = EdmClient.CODE,EdmClient.NAME;
     * String where = EdmClient.CODE + "=<val1";
     * String order = EdmClient.CODE;
     * EdmClient edmClientArray[] =
     *     new EdmClient().get(columns, where, order);</pre>
     * will get the code and name colums of all EdmClient records,
     ...
     * @return Array of EdmClient (EdmClient[])
     */
    public EdmClient[] get (String fields, String where, String order) {
     ...
     * and transforms it into an array of EdmClient.
     * @param  result  (Vector (rows) of Vectors (columns)).
     */
    private EdmClient[] doGet(Vector result) {
        if (dbg) System.out.println ("vector size = " + result.size());
        int codeCol = db.getColNumber(CODE);
        int nameCol = db.getColNumber(NAME);
        EdmClient[] cArray = new EdmClient[result.size()];
        for (int i = 0; i < result.size(); i++) {
            Vector row = (Vector) result.elementAt(i);
            cArray[i] = new EdmClient();
            if (codeCol != -1)
     ...
     *  .e.g.<pre>
     * Boolean success
     * EdmClient edmClient = new EdmClient();
     * success = edmClient.del(EdmClient.CODE+"=<val1>");</pre>
     ...
     * update are taken from the EdmClient argument, .e.g.<pre>
     * Boolean success
     * EdmClient updEdmClient = new EdmClient();
     * updEdmClient.setName(<val2>);
     * EdmClient whereEdmClient = new EdmClient(<val1>);
     * success = whereEdmClient.upd(updEdmClient);</pre>
     * will update Name to <val2> for all records where
     * code = <val1>.
     * @param  edmClient  A EdmClient variable in which the fields are to be used for
     *                 the update are set
     * @return success = true/false (boolean)
     */
    public boolean upd(EdmClient edmClient) {
        return db.update (TABLE, createColVals(edmClient), createWhere());
    } // method upd
     ...
     * EdmClient updEdmClient = new EdmClient();
     * updEdmClient.setName(<val2>);
     * EdmClient whereEdmClient = new EdmClient();
     * success = whereEdmClient.upd(
     *     updEdmClient, EdmClient.CODE+"=<val1>");</pre>
     * will update Name to <val2> for all records where
     * code = <val1>.
     * @param  edmClient  A EdmClient variable in which the fields are to be used for
     *                 the update are set
     ...
     * @return success = true/false (boolean)
     */
    public boolean upd(EdmClient edmClient, String where) {
        return db.update (TABLE, createColVals(edmClient), where);
    } // method upd
     ...
     */
    private String createColVals(EdmClient aVar) {
        String colVals = "";
