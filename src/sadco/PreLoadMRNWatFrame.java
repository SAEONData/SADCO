 ...
 * 040610 - Ursula von St Ange - add ploadtype hidden variable              <br>
 *                               change name from PreLoadMRNFrame to
 *                               PreLoadMRNWatFrame                         <br>
 */
public class PreLoadMRNWatFrame extends CompoundItem {
     ...
     *
     */
    public PreLoadMRNWatFrame(String args[]) {
        ...
        while (!ec.eof(lc.inputFile)) {
            ...
            if ("".equals(line)) {
             ...
             if (dbg) System.out.println("<br>lc.code = " + lc.code);
        ...
        if (lc.fatalCount > 0) {
            ...
            mainTable.addRow(ec.cr1ColRow("<br>"));
            mainTable.addRow(ec.cr1ColRow("error"));
        } // if (lc.fatalCount > 0)
        ...
        if (lc.fatalCount == 0) {
            ...
            if (checkInventory.length == 0) {
                ...
                invTable.addRow(ec.cr2ColRow("Project Name:",
                    sc.PROJECTNAME, 20, 20, lc.projectName, "*"));                      // ub03
                invTable.addRow(ec.cr2ColRow("Cruise Name:",
                    sc.CRUISENAME, 20, 20, lc.expeditionName, "*"));                    // ub03
                ...
                invTable.addRow(ec.cr2ColRow("Institute:",
                    createInstituteSelect(sc.INSTITCODE, lc.instituteName).toHTML()));
                invTable.addRow(ec.cr2ColRow(":", "(" + lc.instituteName + ")"));       // ub03

                invTable.addRow(ec.cr2ColRow("Chief Scientist:",                        // ub03
                    createScientistSelect(sc.SCIENTCODE, lc.chiefScientist).toHTML())); // ub03
                invTable.addRow(ec.cr2ColRow(":", "(" + lc.chiefScientist + ")"));      // ub03

                invTable.addRow(ec.cr2ColRow("Platform:",
                    createPlatformSelect(sc.PLANAMCODE, lc.platformName).toHTML()));
                invTable.addRow(ec.cr2ColRow(":", "(" + lc.platformName + ")"));        // ub03

                invTable.addRow(ec.cr2ColRow("Country:",
                    createCountrySelect(sc.COUNTRYCODE, lc.countryName).toHTML()));
                invTable.addRow(ec.cr2ColRow(":", "(" + lc.countryName + ")"));         // ub03
            ...
            logTable.addRow(ec.cr2ColRow("Source of data:",
                sc.SUBMITSC, 30, 30, lc.submitScientist, "*"));             // ub03
...
*/
    ...
    Container JavaScript(int checkInventoryLength) {
        ...
        con.addItem(JSLLib.CallNotNull(sc.SUBMITSC, "Source of data"));
     ...
     */
    Select createInstituteSelect(String name, String instName) {
        ...
        for (int i = 0; i < institutes.length; i++) {
            ...
            if (instName.equalsIgnoreCase(institutes[i].getName(""))) defaultOption = true;
            list.addOption(new Option(
                institutes[i].getName(""), institutes[i].getCode(""), defaultOption));
        } // for (int i = 0; i < institutes.length; i++)
        return list;
    ...
    Select createPlatformSelect(String name, String platformName) {
        ...
        for (int i = 0; i < planam.length; i++) {
            boolean defaultOption = false;
            if (platformName.equalsIgnoreCase(planam[i].getName(""))) defaultOption = true;
            list.addOption(new Option(
    ...
    /**
     * Create a drop-down box of scientist names                        // ub03
     * @param  name   the parameter name                                // ub03
     * @return  the html Select statement                               // ub03
     */                                                                 // ub03
    Select createScientistSelect(String name, String scientistName) {   // ub03
        Select list = new Select(name);                                 // ub03
        MrnScientists scientists[] = new MrnScientists().get(           // ub03
            "1=1",MrnScientists.SURNAME);                               // ub03
        for (int i = 0; i < scientists.length; i++) {                   // ub03
            boolean defaultOption = false;                              // ub03
            String scientist = scientists[i].getSurname("") + ", " +    // ub03
                scientists[i].getTitle("") + " " +                      // ub03
                scientists[i].getFName("");                             // ub03
            scientistName = scientistName.toLowerCase();                // ub03
            String surname = scientists[i].getSurname("").toLowerCase();// ub03
            if (scientistName.indexOf(surname) > 0) defaultOption = true;// ub03
            list.addOption(new Option(                                  // ub03
                scientist, scientists[i].getCode(""), defaultOption));  // ub03
        } // for (int i = 0; i < scientists.length; i++)                // ub03
        return list;                                                    // ub03
    } // createScientistSelect                                          // ub03
     ...
     * @return  the html Select statement
     */
    Select createCountrySelect(String name, String countryName) {
        Select list = new Select(name);
        MrnCountry country[] = new MrnCountry().get(
            "1=1",MrnCountry.NAME);
        for (int i = 0; i < country.length; i++) {
            boolean defaultOption = false;
            if (countryName.equalsIgnoreCase(country[i].getName(""))) defaultOption = true;
            list.addOption(new Option(
                country[i].getName(""), country[i].getCode(""), defaultOption));
        } // for (int i = 0; i < country.length; i++)
        return list;
    } // createCountrySelect
    ...
    void displayStationDetails() {
        ...
        for (int i = 0; i < lc.tStation.length; i++) {
            ...
            if (lc.stationExistsArray[i]) {
                ...
                dupTable.addRow(ec.cr3ColRow(
                    formatStr("No of Currents Records:"+sp),
                    formatStr(sp+lc.currentsRecordCountArray[i]),
                    formatStr(sp+0)));
                dupTable.addRow(ec.cr3ColRow(
                    formatStr("No of Sedphy Records:"+sp),
                    formatStr(sp+lc.sedphyRecordCountArray[i]),
                    formatStr(sp+0)));
                dupTable.addRow(ec.cr3ColRow(
                    formatStr("No of Wathy Records:"+sp),
        ...
        //        } // if (lc.stationStatus2.startswith("ds"))
                dupTable.addRow(ec.crSpanColsRow(
                    ...
                    lc.watphy.getSubdes("") + ") : " + lc.subdesCount[i] + "<b>"), 3));
                dupTable.addRow(ec.crSpanColsRow(
                ...
                if (lc.subdesCount[i] > 0) {
                    dupTable.addRow(ec.crSpanColsRow(formatStr(
                        "Replace the DATA (Watphy) records for this station? " +
...
/** utility functions
    ...
    public static void main(String args[]) {
        ...
        try {
            HtmlHead hd = new HtmlHead("PreLoadMRNWatFrame local");
            HtmlBody bd = new HtmlBody();
            HtmlPage hp = new HtmlPage(hd,bd);
            //PreLoadMRNWatFrame local= new PreLoadMRNWatFrame(args);
            bd.addItem(new PreLoadMRNWatFrame(args));
            hp.printHeader();
            hp.print();
        } catch(Exception e) {
            ec.processErrorStatic(e, "PreLoadMRNWatFrame", "Constructor", "");
