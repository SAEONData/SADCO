 ...
 */
public class ExtractMRNPhysNutDataPedro { //extends CompoundItem {
    ...
    /**
     * Main constructor ExtractMRNPhysNutDataPedro
     * @param   args   String array of arguments as from form or URL
     */
    public ExtractMRNPhysNutDataPedro(String args[])  {
    ...
    } // ExtractMRNPhysNutDataPedro(String args[])
    ...
    MrnStation[] getStationRecords() {
        ...
        where += " and (" + MrnStation.STNDEP + " >= 500 or " +
            MrnStation.MAX_SPLDEP + " >= 500)";
    ...
    int getDataRecords(MrnStation[] stations) {
        ...
        for (int i = 0; i < stations.length; i++) {
            ...
            if (watphy2.length > 0) {
                ...
                if (maxSpldep > 400f) maxSpldep = 400f;                 // for pedro
            } // if (watphy2.length > 0)
            ...
            for (int k = 0; k <= maxSpldep; k += spldepInterval) {
                ...
                where =
                    ...
                    //" and salinity between 34.4 and 35";                        //for pedro
                MrnWatphy[] watphy = new MrnWatphy().get(fields, where, order);
                if (dbg) System.out.println("<br>i = " + i + ", k = " + k +
                    ", where = " + where +
                ...
                for (int j = 0; j < watphy.length; j++) {
                    ...
                    if (dbg) System.out.println("<br>j = " + j +
                        ...
                        ", spldep = " + watphy[j].getSpldep() +
                        ", disoxygen = *" + watphy[j].getDisoxygen("") +
                        "*" + watphy[j].getDisoxygen() + "*");

                    if ((watphy[j].getSpldep() <= maxSpldep) && ((watnut.length > 0) ||
                            (!"".equals(watphy[j].getDisoxygen(""))))) {
    ...
    void printRecords(MrnStation station, MrnWatphy watphy, MrnPlanam[] planam,
        ...
        float sio4 = MrnWatnut.FLOATNULL;
        if (watnut.length > 0) {
            no2  = watnut[0].getNo2();
            no3  = watnut[0].getNo3();
            po4  = watnut[0].getPo4();
        ...
        String line =
            ...
            ec.frm(ec.nullToNines(sio4,9999.0f),10,2) + "," +
    ...
    void ColumnHeadings() {
        ...
        String head = " LATDEG ,  LONDEG, YEAR, MONTH, DEPTH,DAY,    TIME,   TMP,    "+
                      ...
                      "SIO3,      SIO4,      PH,   SOUNDV,      CHLA,    STATION ID,    "+
                      "STNDEP,  MXSPLDEP,     STN NAM,   SURVEYID,                " +
                      "PLANAM,  INSTR";
    ...
    void printLegend() {
        ...
        String legend = "\n\n" +
            ...
            "  SIO4      =   SILICATE (MICROGRAM ATOM/LITRE) (NO VALUE = 9999.0)\n" +
            "  PH        =   PH (NO VALUE = 99.0)\n" +
    ...
    public static void main(String args[]) {
        ...
        try {
            ExtractMRNPhysNutDataPedro local =
                new ExtractMRNPhysNutDataPedro(args);
        } catch(Exception e) {
            common.edmCommonPC.processErrorStatic(
                e, "ExtractMRNPhysNutDataPedro", "Constructor", "");
        } // try-catch
 ...
 }// class ExtractMRNPhysNutDataPedro
