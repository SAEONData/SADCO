...
/**
 * This class manages the WAV_PERIOD table with the help of the DbAccessC
 ...
 */
public class WavPeriod extends Tables {
    ...
    /** The table name */
    public static final String TABLE = "WAV_PERIOD";
    ...
    public WavPeriod() {
        clearVars();
        if (dbg) System.out.println ("<br>in WavPeriod constructor 1"); // debug
    } // WavPeriod constructor

    /**
     * Instantiate a WavPeriod object and initialize the instance variables.
    ...
    public WavPeriod(
        ...
        if (dbg) System.out.println ("<br>in WavPeriod constructor 2"); // debug
    } // WavPeriod constructor
    ...
    public WavPeriod(
        ...
        setM11       (m11      );
        setM12       (m12      );
        if (dbg) System.out.println ("<br>in WavPeriod constructor 3"); // debug
    } // WavPeriod constructor
     ...
     * are returned in an array of WavPeriod. e.g.<pre>
     * WavPeriod wavPeriod = new WavPeriod(<val1>);
     * WavPeriod wavPeriodArray[] = wavPeriod.get();</pre>
     * will get the WavPeriod record where stationId = <val1>.
     * @return Array of WavPeriod (WavPeriod[])
     */
    public WavPeriod[] get() {
     ...
     * e.g.<pre>
     * WavPeriod wavPeriodArray[] = 
     *     new WavPeriod().get(WavPeriod.STATION_ID+"=<val1>");</pre>
     * will get the WavPeriod record where stationId = <val1>.
     * @param  where  The where clause for the select statement. (String)
     *     If where == "" / null, it is ignored.
     * @return Array of WavPeriod (WavPeriod[])
     */
    public WavPeriod[] get (String where) {
     ...
     * WavPeriod wavPeriodArray[] = 
     *     new WavPeriod().get("1=1",wavPeriod.STATION_ID);</pre>
     * will get the all the WavPeriod records, and order them by stationId.
     * @param  where  The where clause for the select statement. (String)
     *     If where == "" / null, it is ignored.
     ...
     * @return Array of WavPeriod (WavPeriod[])
     */
    public WavPeriod[] get (String where, String order) {
     ...
     * String columns = WavPeriod.STATION_ID,WavPeriod.YEARP;
     * String where = WavPeriod.STATION_ID + "=<val1";
     * String order = WavPeriod.STATION_ID;
     * WavPeriod wavPeriodArray[] = 
     *     new WavPeriod().get(columns, where, order);</pre>
     * will get the stationId and yearp colums of all WavPeriod records,
     ...
     * @return Array of WavPeriod (WavPeriod[])
     */
    public WavPeriod[] get (String fields, String where, String order) {
     ...
     * and transforms it into an array of WavPeriod.
     * @param  result  (Vector (rows) of Vectors (columns)).
     */
    private WavPeriod[] doGet(Vector result) {
        ...
        WavPeriod[] cArray = new WavPeriod[result.size()];
        for (int i = 0; i < result.size(); i++) {
            Vector row = (Vector) result.elementAt(i);
            cArray[i] = new WavPeriod();
            if (stationIdCol != -1)
     ...
     *  .e.g.<pre>
     * Boolean success
     * WavPeriod wavPeriod = new WavPeriod();
     * success = wavPeriod.del(WavPeriod.STATION_ID+"=<val1>");</pre>
     ...
     * update are taken from the WavPeriod argument, .e.g.<pre>
     * Boolean success
     * WavPeriod updWavPeriod = new WavPeriod();
     * updWavPeriod.setYearp(<val2>);
     * WavPeriod whereWavPeriod = new WavPeriod(<val1>);
     * success = whereWavPeriod.upd(updWavPeriod);</pre>
     * will update Yearp to <val2> for all records where 
     * stationId = <val1>.
     * @param wavPeriod  A WavPeriod variable in which the fields are to be used for 
     *                 the update are set
     * @return success = true/false (boolean)
     */
    public boolean upd(WavPeriod wavPeriod) {
        return db.update (TABLE, createColVals(wavPeriod), createWhere());
    } // method upd
     ...
     * WavPeriod updWavPeriod = new WavPeriod();
     * updWavPeriod.setYearp(<val2>);
     * WavPeriod whereWavPeriod = new WavPeriod();
     * success = whereWavPeriod.upd(
     *     updWavPeriod, WavPeriod.STATION_ID+"=<val1>");</pre>
     * will update Yearp to <val2> for all records where 
     * stationId = <val1>.
     * @param  updWavPeriod  A WavPeriod variable in which the fields are to be used for 
     *                 the update are set
     * @param  where  The where clause for the select statement. (String)
     * @return success = true/false (boolean)
     */
    public boolean upd(WavPeriod wavPeriod, String where) {
        return db.update (TABLE, createColVals(wavPeriod), where);
    } // method upd
     ...
     */
    private String createColVals(WavPeriod aVar) {
        String colVals = "";
