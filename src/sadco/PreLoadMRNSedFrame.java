 ...
 * @version
 * 060315 - Ursula von St Ange - adapt from PreLoadMRNSedFrame              <br>
 */
public class PreLoadMRNSedFrame extends CompoundItem {
     ...
     *
     */
    public PreLoadMRNSedFrame(String args[]) {
...
*/
    ...
    void displayStationDetails() {
        ...
        for (int i = 0; i < lc.tStation.length; i++) {
            ...
            if (lc.stationExistsArray[i]) {
                ...
                dupTable.addRow(ec.cr3ColRow(
                    ...
                    formatStr(sp+0)));
                dupTable.addRow(ec.cr3ColRow(
                    formatStr("No of Sedhy Records:"+sp),
        ...
        //                formatStr(sp+lc.tSedphy[0].getSpldattim("")),
        //                formatStr(sp+sedphy.getSpldattim(""))));
        //        } // if (lc.stationStatus2.startswith("ds"))
                dupTable.addRow(ec.crSpanColsRow(
                    formatStr("Number of records with same SUBDES </b>(" +
                    lc.sedphy.getSubdes("") + ") : " + lc.subdesCount[i] + "<b>"), 3));
                dupTable.addRow(ec.crSpanColsRow(
                ...
                if (lc.subdesCount[i] > 0) {
                    dupTable.addRow(ec.crSpanColsRow(formatStr(
                        "Replace the DATA (Sedphy) records for this station? " +
...
/** utility functions
    ...
    public static void main(String args[]) {
        ...
        try {
            HtmlHead hd = new HtmlHead("PreLoadMRNSedFrame local");
            HtmlBody bd = new HtmlBody();
            HtmlPage hp = new HtmlPage(hd,bd);
            //PreLoadMRNSedFrame local= new PreLoadMRNSedFrame(args);
            bd.addItem(new PreLoadMRNSedFrame(args));
            hp.printHeader();
            hp.print();
        } catch(Exception e) {
            ec.processErrorStatic(e, "PreLoadMRNSedFrame", "Constructor", "");
