 ...
 */
public class ProductVOSGetFileFrame extends CompoundItem{
     ...
     */
    public ProductVOSGetFileFrame (String args[]) {

        try {
            ProductVOSGetFileFrameActual(args);
        } catch(Exception e) {
            ec.processError(e, this.getClass().getName(), "constructor", "");
        } // try - catch

    } // ProductVOSGetFileFrame constructor
     ...
     * @param args the string array of url-type name-value parameter pairs
     */
    void ProductVOSGetFileFrameActual(String args[]) {
        ...
        for (int i = 0; i < 6; i++) {
            productType.addOption(
                new Option(sc.VOSPRODLIST[i], String.valueOf(i+1), false));
        } // for (int i
