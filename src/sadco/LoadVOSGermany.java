...
/**
 * Does the loading of VOS_MAIN data as received from Germany (199206 - 199308)
 * data in IMMT-3 format
 *
 * standalone program to load once-off data
 *
 * @author  111201 - SIT Group
 * @version
 * 111201 - Ursula von St Ange - Adapted from LoadVOSData2.java         <br>
 */
public class LoadVOSGermany extends CompoundItem {
    ...
    String      sts;       // Sign of sea-surface temperature (stemp 00 ?) - WMO code table 3845
    ...
    String      obsSource; // Source of observation
    String      platform;  // Observation platform
    String      callsign;  // Ship's Call sign (c_callsn)
    //String      country;   // Country which has recruited the ship

    //String      qci;       // Quality control indicator
    //String      xi;        // Weather data indicator
    //String      ri;        // Indicator for inclusion or ommission of precipitation data WMO code table 1819
    //String      rrr;       // Amount of precipitation WMO code table 3590
    //String      tr;        // Duration of period of reference for amount of precipitation
    ...
    // error flags
    boolean     readErrorFlag = false;  // date, lat, lon, quadrant, overland testing
    ...
    */
    ...
    public LoadVOSGermany(String args[]) {

        if (dbg) System.out.println("<br>LoadVOSGermany: before getParms");
        // get the argument values
        getArgParms(args);

        if (dbg) System.out.println("<br>LoadVOSGermany: After getParms");
    ...
    void readData() {
        ...
        dataFormat = line.substring(0,1);    // 00 or 03

        year     = line.substring(1,5);
        month    = line.substring(5,7);
        day      = line.substring(7,9);    // dag
        hour     = line.substring(9,11);   // tyd
        quadrant = line.substring(11,12);  //
        latin    = line.substring(12,15);  // slat
        longin   = line.substring(15,19);  // slog
        ...
        if (dbg) System.out.println();
        if (dbg) System.out.print("readData: quadrant = " + quadrant);
        if (dbg) System.out.print(", latin  = " + latin);
        if (dbg) System.out.print(", longin = " + longin);
        if (dbg) System.out.println();

        clht  = line.substring(20,21);    // stts
        vv    = line.substring(21,23);
        clfr  = line.substring(23,24);    // windg

        if (dbg) System.out.print("readData: clht  = " + clht);
        if (dbg) System.out.print(", vv    = " + vv);
        if (dbg) System.out.print(", clfr  = " + clfr);
        if (dbg) System.out.println();

        wdir  = line.substring(24,26);
        wi    = line.substring(26,27);    // wndc
        wsp   = line.substring(27,29);

        if (dbg) System.out.print("readData: wdir = " + wdir);
        if (dbg) System.out.print(", wi       = " + wi);
        if (dbg) System.out.print(", wsp  = " + wsp);
        if (dbg) System.out.println();

        ats   = line.substring(29,30);    // tts
        at    = line.substring(30,33);
        dws   = line.substring(33,34);    // tds
        dw    = line.substring(34,37);
        ...
        slp   = line.substring(37,41);    // c_press
        ppwn  = line.substring(41,43);    // c_app
        ctf   = line.substring(45,46);    // c_ww
        ctl   = line.substring(46,47);
        ctm   = line.substring(47,48);
        cth   = line.substring(48,49);

        if (dbg) System.out.print("readData: slp   = " + slp);
        ...
        sts   = line.substring(49,50);
        st    = line.substring(50,53);

        wwp   = line.substring(55,57);
        wwh   = line.substring(57,59);
        sw1d  = line.substring(59,61);
        sw1p  = line.substring(61,63);
        sw1h  = line.substring(63,65);

        if (dbg) System.out.print("readData: sts  = " + sts );
        ...
        obsSource = line.substring(69,70);    // c_callsn
        platform  = line.substring(70,71);    // c_callsn
        callsign  = line.substring(71,78);    // c_callsn

        wts   = line.substring(88,89);    // wet bulb
        wt    = line.substring(89,92);
        if (dbg) System.out.print("readData: obsS  = " + obsSource);
        if (dbg) System.out.print(", platf = " + platform);
...
//        } //if (testno < 50) {
        ...
        testReadErrorValue(year, 1992, 1993, "YEAR   ", "Invalid year");
        ...
        // 5 == South, 0 - 90 West
        // 7 == North, 0 - 90 West
        int intQ = getValue(quadrant);
        if ((intQ != 1) && (intQ != 3) && (intQ != 5) && (intQ != 7)) {
            setReadErrorFlags("Q      ", quadrant, "Invalid quadrant");
        } // if ((intQ != 1) && (intQ != 3) && (intQ != 5) && (intQ != 7)) {
        String quad = "";
        switch (intQ) {
        ...
        // LATITUDE - if quadrant is north (Q=0 or 3) the value must be -ve
        latinS = "";
        if ((intQ == 1) || (intQ  == 7)) latinS = "-"; // north
        ...
        */
        ...
        // LONGITUDE - if quadrant is west (Q=0 or 5) the value must be -ve
        loninS = "";
        if ((intQ == 5) || (intQ == 7)) loninS = "-"; // west
    ...
    void processRest(VosTable data) {
        ...
        int directionMax = 36;
        float directionMult = 10f;
        boolean type00 = false;
        ...
        if ((testno != 0) && (testno != 1) && (testno != 3) && (testno != 4)) {
            ...
            // WIND SPEED TYPE - Codes 0 or 2 indicate estimated reading
            // Codes 1 and 3 are anenometer readings. Use code E for
        ...
        if (!tempErrorFlag) {
            ...
            if (!tempErrorFlag) {
                ...
                if ((testno == 2) || (testno == 3)) ftestno *= 0.51f;
                if (dbg) System.out.println("<br>windspeed = " + ftestno);
        ...
        // convert + and - to 0 and 1
        //testno = convertPlusMinus(EVAL, ats, 0, 1);
        wts = wts.replace('+', '0').replace('-', '1').replace('5', '0').replace('6', '1');;
        ...
        dws = dws.replace('+', '0').replace('-', '1').replace('5', '0').replace('6', '1');
        ...
        if (!tempErrorFlag) {
            ...
            if (dbg) System.out.println("<br>processRest: **??** slp "+slp);
            atmosCount[0]++;
            if ("0".equals(slp.substring(0,1))) {
                //ftestno += 1000f;
                slp ="1"+slp;
            } // if ("0".equals(slp.substring(0,1)))
            ftestno = Float.parseFloat(slp) / 10f;
...
//        } // if (testno != EVAL)
        ...
        if (rejected) {
            ...
            reject.setDataId("DW");   //SAWS
            reject.setQualityControl("9");
            reject.setSource1(obsSource);
...
*/
    ...
    public static void main(String args[]) {
        ...
        try {
            LoadVOSGermany local= new LoadVOSGermany(args);
        } catch(Exception e) {
            //ec.processErrorStatic(e, "LoadVOSGermany", "Constructor", "");
            e.printStackTrace();
