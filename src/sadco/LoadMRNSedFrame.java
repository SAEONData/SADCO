 ...
 * @version
 * 060315 - Ursula von St Ange - adapt from LoadMRNSedFrame             <br>
 */
public class LoadMRNSedFrame extends CompoundItem {
     ...
     */
    public LoadMRNSedFrame (String args[]) {

        try {
            LoadMRNSedFrameActual(args);
        } catch(Exception e) {
            ec.processError(e, this.getClass().getName(), "constructor", "");
        } // try - catch

    } // LoadMRNSedFrame constructor
     ...
     * @param args the string array of url-type name-value parameter pairs
     */
    void LoadMRNSedFrameActual(String args[]) {
        ...
        String command = sc.JAVA + " " + sc.MRN_APP + "Apps " +
            sc.SCREEN + "=load " +
            sc.VERSION + "=doload " +
            sc.LOADTYPE + "=" + ec.getArgument(args, sc.LOADTYPE) + " " +
            sc.FILENAME + "=" + fileName;

        //if (dbg) System.out.println("<br>command = " + command);
        if (dbg) System.err.println("** LoadMRNSedFrame command = " + command);
    ...
    public static void main(String args[]) {
        ...
        try {
            HtmlHead hd = new HtmlHead("LoadMRNSedFrame local");
            HtmlBody bd = new HtmlBody();
            HtmlPage hp = new HtmlPage(hd,bd);
            //PreLoadMRNSedFrame local= new PreLoadMRNSedFrame(args);
            bd.addItem(new LoadMRNSedFrame(args));
            hp.printHeader();
            hp.print();
        } catch(Exception e) {
            ec.processErrorStatic(e, "LoadMRNSedFrame", "Constructor", "");
