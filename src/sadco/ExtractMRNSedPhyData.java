 ...
 */
public class ExtractMRNSedPhyData { //extends CompoundItem {
    ...
    /**
     * Main constructor ExtractMRNSedPhyData
     * @param   args   String array of arguments as from form or URL
     */
    public ExtractMRNSedPhyData(String args[])  {
        ...
        if (dbg && dbg2File) {
            String dbgFileName = rootPath + "ExtractMRNSedPhyData.dbg";
    ...
    } // ExtractMRNSedPhyData(String args[])
...
/*
                ...
                // get the sediment chemistry 1 data
                String fields2 = "*";
                ...
                // get the sediment chemistry 2 data
...
//                if ((sedchem1.length > 0) || (sedchem2.length > 0)) {
                printRecords(stations[i], sedphy[j]);
                noRecords += 1;
//                } // if ((sedchem1.length > 0) !! (sedchem1.length > 0))
     ...
     */
    void printRecords(MrnStation station, MrnSedphy sedphy) {
        ...
        int       spldis = sedphy.getSpldis();
        float     splvol = sedphy.getSplvol();
        float     sievsz = sedphy.getSievsz();
        float     kurt   = sedphy.getKurt();
        float     skew   = sedphy.getSkew();
        int       meanpz = sedphy.getMeanpz();
        int       medipz = sedphy.getMedipz();
        float     pctsat = sedphy.getPctsat();
        float     pctsil = sedphy.getPctsil();
        int       permty = sedphy.getPermty();
        float     porsty = sedphy.getPorsty();
        float     dwf    = sedphy.getDwf();
        float     cod    = sedphy.getCod();
        ...
        String line =
            ...
            ec.frm(ec.nullToNines(spldis,9999   ), 9  ) + "," +
            ec.frm(ec.nullToNines(splvol,9999.0f), 9,3) + "," +
            ec.frm(ec.nullToNines(sievsz,9999.0f), 9,3) + "," +
            ec.frm(ec.nullToNines(kurt  ,9999.0f), 9,3) + "," +
            ec.frm(ec.nullToNines(skew  ,9999.0f), 9,3) + "," +
            ec.frm(ec.nullToNines(meanpz,9999   ), 9  ) + "," +
            ec.frm(ec.nullToNines(medipz,9999   ), 9  ) + "," +
            ec.frm(ec.nullToNines(pctsat,9999.0f), 9,3) + "," +
            ec.frm(ec.nullToNines(pctsil,9999.0f), 9,3) + "," +
            ec.frm(ec.nullToNines(permty,9999   ), 9  ) + "," +
            ec.frm(ec.nullToNines(porsty,9999.0f), 9,3) + "," +
            ec.frm(ec.nullToNines(dwf   ,9999.0f), 9,3) + "," +
            ec.frm(ec.nullToNines(cod   ,9999.0f), 9,3) + ",\"" +
    ...
    void ColumnHeadings() {
        String head = "  LATDEG,  LONDEG, YEAR,MNTH,   DEPTH,DAY,    TIME," +
            "   SPLDIS,   SPLVOL,   SIEVSZ,     KURT,     SKEW,   MEANPZ," +
            "   MEDIPZ,   PCTSAT,   PCTSIL,   PERMTY,   PORSTY,      DWF," +
            "      COD," +
    ...
    void printLegend() {
        ...
        String legend = "\n\n" +
            ...
            "  SPLDIS    =   SPLDIS (MICROGRAM/GRAM) (NO VALUE = 9999.0)\n" +
            "  SPLVOL    =   SAMPLE VOLUME (LITRE) (NO VALUE = 9999.0)\n" +
            "  SIEVSZ    =   SIEV SIZE (NO VALUE = 9999.0)\n" +
            "  KURT      =   PARTICLE DISTRIB KURTOSES (MICRO NS) (NO VALUE = 9999.0)\n" +
            "  SKEW      =   PARTICLE DISTRIB SKEWNESS (MICRO NS) (NO VALUE = 9999.0)\n" +
            "  MEANPZ    =   MEAN PARTICLE SIZE (NO VALUE = 9999.0)\n" +
            "  MEDIPZ    =   MEDIAN PARTICAL SIZE (NO VALUE = 9999.0)\n" +
            "  PCTSAT    =   PERCENT SATURATIONS (NO VALUE = 9999.0)\n" +
            "  PCTSIL    =   PERCENT SILT < 63 MICORGRAM (DRY MASS) (NO VALUE = 9999.0)\n" +
            "  PERMTY    =   PERMEABILITY (NO VALUE = 9999.0)\n" +
            "  PORSTY    =   POROSITY (DRY MASS) (NO VALUE = 9999.0)\n" +
            "  DWF       =   DRY WEIGHT FACTOR (NO VALUE = 9999.0)\n" +
            "  COD       =   CHEMICAL OXYGEN DEMAND (MG O2/L) (NO VALUE = 9999.0)\n" +
    ...
    LogUsers createLogUsers(String userid, Timestamp start, String timeDiff) {//ub04
        ...
        logs.setDiscipline("sedphy");                                   //ub04
...
*/
    ...
    public static void main(String args[]) {
        ...
        try {
            ExtractMRNSedPhyData local =
                new ExtractMRNSedPhyData(args);
        } catch(Exception e) {
            common.edmCommonPC.processErrorStatic(
                e, "ExtractMRNSedPhyData", "Constructor", "");
        } // try-catch
