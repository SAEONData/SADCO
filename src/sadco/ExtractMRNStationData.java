 ...
 */
public class ExtractMRNStationData {
    ...
    /**
     * Main constructor ExtractMRNStationData
     * @param   args   String array of arguments as from form or URL
     */
    public ExtractMRNStationData(String args[])  {
...
//        } // if (validQuery)
    ...
    } // ExtractMRNStationData(String args[])
...
//'NMI40196001 ', '1996/01/14',  28.017,   15.550,     90.000
        ...
        ec.writeFileLine(ofile, "'" +
            (!"/".equals(surveyId) ? surveyId : "Various") + "' " +
            station.length + " '" +
            shipName + "' '" +
            projectName + "' '" +
            cruiseName + "'");
        ec.writeFileLine(ofile, "'" + institute + "'");
        ...
        for (int i = 0; i < station.length; i++) {
            ...
            ec.writeFileLine(ofile, "'" +
                ec.frm(station[i].getStationId(""),12) + "' '" +
                station[i].getDateStart("").substring(0,10) + "' " +
                ec.frm(station[i].getLatitude(),11,5) + " " +
                ec.frm(station[i].getLongitude(),11,5) + " "  +
                ec.frm(ec.nullToNines(station[i].getStndep(),9999.0f),10,2) + " " +
                ec.frm(ec.nullToNines(etopo2Depth,9999.0f),10,2) + " '" +    //ub06
                ec.frm(station[i].getStnnam(""),10) + "'");
        } // for (int i = 0; i < station.length; i++)
