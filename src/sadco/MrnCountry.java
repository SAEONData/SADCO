...
/**
 * This class manages the COUNTRY table with the help of the DbAccessC
 ...
 */
public class MrnCountry extends Tables {
    ...
    public MrnCountry() {
        clearVars();
        if (dbg) System.out.println ("<br>in MrnCountry constructor 1"); // debug
    } // MrnCountry constructor

    /**
     * Instantiate a MrnCountry object and initialize the instance variables.
    ...
    public MrnCountry(
        ...
        this();
        setCode (code);
        if (dbg) System.out.println ("<br>in MrnCountry constructor 2"); // debug
    } // MrnCountry constructor
    ...
    public MrnCountry(
        ...
        setCode (code);
        setName (name);
        if (dbg) System.out.println ("<br>in MrnCountry constructor 3"); // debug
    } // MrnCountry constructor
     ...
     * are returned in an array of MrnCountry. e.g.<pre>
     * MrnCountry country = new MrnCountry(<val1>);
     * MrnCountry countryArray[] = country.get();</pre>
     * will get the MrnCountry record where code = <val1>.
     * @return Array of MrnCountry (MrnCountry[])
     */
    public MrnCountry[] get() {
     ...
     * e.g.<pre>
     * MrnCountry countryArray[] =
     *     new MrnCountry().get(MrnCountry.CODE+"=<val1>");</pre>
     * will get the MrnCountry record where code = <val1>.
     * @param  where  The where clause for the select statement. (String)
     *     If where == "" / null, it is ignored.
     * @return Array of MrnCountry (MrnCountry[])
     */
    public MrnCountry[] get (String where) {
     ...
     * MrnCountry countryArray[] =
     *     new MrnCountry().get("1=1",country.CODE);</pre>
     * will get the all the MrnCountry records, and order them by code.
     * @param  where  The where clause for the select statement. (String)
     *     If where == "" / null, it is ignored.
     ...
     * @return Array of MrnCountry (MrnCountry[])
     */
    public MrnCountry[] get (String where, String order) {
     ...
     * String columns = MrnCountry.CODE,MrnCountry.NAME;
     * String where = MrnCountry.CODE + "=<val1";
     * String order = MrnCountry.CODE;
     * MrnCountry countryArray[] =
     *     new MrnCountry().get(columns, where, order);</pre>
     * will get the code and name colums of all MrnCountry records,
     ...
     * @return Array of MrnCountry (MrnCountry[])
     */
    public MrnCountry[] get (String fields, String where, String order) {
     ...
     * and transforms it into an array of MrnCountry.
     * @param  result  (Vector (rows) of Vectors (columns)).
     */
    private MrnCountry[] doGet(Vector result) {
        if (dbg) System.out.println ("vector size = " + result.size());
        int codeCol = db.getColNumber(CODE);
        int nameCol = db.getColNumber(NAME);
        MrnCountry[] cArray = new MrnCountry[result.size()];
        for (int i = 0; i < result.size(); i++) {
            Vector row = (Vector) result.elementAt(i);
            cArray[i] = new MrnCountry();
            if (codeCol != -1)
     ...
     * Boolean success = new MrnCountry(
     *     Tables.INTNULL,
     *     <val2>,
     ...
     *  .e.g.<pre>
     * Boolean success
     * MrnCountry country = new MrnCountry();
     * success = country.del(MrnCountry.CODE+"=<val1>");</pre>
     ...
     * update are taken from the MrnCountry argument, .e.g.<pre>
     * Boolean success
     * MrnCountry updMrnCountry = new MrnCountry();
     * updMrnCountry.setName(<val2>);
     * MrnCountry whereMrnCountry = new MrnCountry(<val1>);
     * success = whereMrnCountry.upd(updMrnCountry);</pre>
     * will update Name to <val2> for all records where
     * code = <val1>.
     * @param  country  A MrnCountry variable in which the fields are to be used for
     *                 the update are set
     * @return success = true/false (boolean)
     */
    public boolean upd(MrnCountry country) {
        return db.update (TABLE, createColVals(country), createWhere());
    } // method upd
     ...
     * MrnCountry updMrnCountry = new MrnCountry();
     * updMrnCountry.setName(<val2>);
     * MrnCountry whereMrnCountry = new MrnCountry();
     * success = whereMrnCountry.upd(
     *     updMrnCountry, MrnCountry.CODE+"=<val1>");</pre>
     * will update Name to <val2> for all records where
     * code = <val1>.
     * @param  country  A MrnCountry variable in which the fields are to be used for
     *                 the update are set
     ...
     * @return success = true/false (boolean)
     */
    public boolean upd(MrnCountry country, String where) {
        return db.update (TABLE, createColVals(country), where);
    } // method upd
     ...
     */
    private String createColVals(MrnCountry aVar) {
        String colVals = "";
