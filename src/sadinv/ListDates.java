...
/**
 * This class handles the search on dates

 * SadInv
 * pscreen = dates
 * pdatestart = <yyyy-mm-dd>
 * psurveyid = <survey id>
 *
 * @author 20090110 - SIT Group.
 * @version
 * 20090110 - Ursula von St Ange - create program                       <br>
 * 20090602 - Ursula von St Ange - add code to enter <yyyy-mm> ub01     <br>
...
public class ListDates extends CompoundItem {
    ...
    // | MAIN PROGRAM                                               |
    // +------------------------------------------------------------+
    public ListDates(String args[]) {

        // +------------------------------------------------------------+
        // | Get input paramaters                                       |
        // +------------------------------------------------------------+
        String dateStart = ec.getArgument(args,sc.DATESTART);
        if (dbg) System.out.println("<br>dateStart = " + dateStart);
        ...
        frm
            .addItem(new SimpleItem("Enter a Starting Date and press <b>ENTER</b>: "))
            .addItem(new Hidden(sc.SCREEN, sc.DATES))
            .addItem(new Hidden(sc.PSC, sessionCode))                   //ub03
            .addItem(new TextField(sc.DATESTART, 10, 10, ""))
            .addItem(new SimpleItem(" (YYYY-MM-DD)"))
            ;
        this.addItem(frm);
        ...
        try {
            ...
            System.out.println("<br>ListDates: connected to postgres");
        ...
        */
        ...
        if (!"".equals(dateStart)) {
            ...
            if (dateStart.length() == 4) {
                dateStart = dateStart +"-01-01";
            } // if (dateStart.length() == 4)
            if (dateStart.length() == 7) {                                  // ub01
                dateStart = dateStart +"-01";                               // ub01
            } // if (dateStart.length() == 7)                               // ub01
            if (dbg) System.out.println("<br>dateStart = " + dateStart);
            ...
            String institNames[]     = new String[MAX_RECS];
            ...
            try {
                ...
                if (dbg) System.out.println("<br> ## numRecs = "+numRecs);
                if (dbg) System.out.println("<br> ## surveyIds[i] = "+surveyIds[i]);

                sql =
                    " select "+
                    " inv.survey_id, inv.project_name, inv.cruise_name, "+
                    " pl.name, sc.surname, sc.f_name, sc.title, inst.name,"+
                    " to_char(inv.date_start,'YYYY-MM-DD'), " +
                    " to_char(inv.date_end,'YYYY-MM-DD'), " +
                    " inv.data_recorded, st.name, inv.data_available" + //ub03 "
                    " from inventory inv, planam pl, institutes inst, scientists sc, "+
                    " survey_type st "+
                    " where inv.date_start >= to_timestamp('"+dateStart+"','YYYY-MM-DD') "+
                    //" where inv.date_start >= to_timestamp('"+dateStart+"','YYYY-MM-DD') "+
                    " and pl.code = inv.planam_code "+
                    " and inst.code = inv.instit_code "+
                    " and sc.code = inv.sci_code_1 "+
                    " and inv.survey_type_code = st.code "+
                    " order by inv.date_start, inv.date_end ";
                ...
                while (rset.next()) {                                     //ub06
                    if (i < MAX_RECS) {                                   //ub06
                        ...
                        if (found) {
                            ...
                            institNames [i] = checkNull(rset.getString(8));
                            ...
                            if (dbg) System.out.println("<br>i "+i+" surveyIds["+i+"] = "+surveyIds[i]);
                            if (dbg) System.out.println("<br>*surnames["+i+"] = "+surnames[i]);
                            if (dbg) System.out.println("<br>*fNames["+i+"] = "+fNames[i]);
                            if (dbg) System.out.println("<br>*titles["+i+"] = "+titles[i]);
                            i++;
                        } // if (found)
                    } else {                                                          //ub06
            ...
            for (int i = 0; i < numRecs; i++) {
                ...
                if (dbg) System.out.println("<br> In for loop to print record line!1 ");
                if (dbg) System.out.println("<br> 2 ");
                ...
                if (dbg) System.out.println("<br> 3 ");
                ...
                rows.addCell(new TableDataCell(formatStr(institNames   [i])));
    ...
    public static void main(String args[]) {
        try {
            HtmlHead hd = new HtmlHead("ListDates local");
