...
/**
 * This class handles the search on platforms - displays the list of
 * surveys for the platform
 *
 * SadInv
 * pscreen = platforms2
 * pplatform = <platform code>
 * psurveyid = <survey id>
 *
...
public class ListPlatform extends CompoundItem {
    ...
    // | MAIN PROGRAM                                               |
    // +------------------------------------------------------------+
    public ListPlatform (String args[]) {

        // +------------------------------------------------------------+
        // | Get input paramaters                                       |
        // +------------------------------------------------------------+
        String platformCode = ec.getArgument(args,sc.CODE);
        if (dbg) System.out.println("<br>platformCode "+platformCode);
        ...
        */
        ...
        // +-------------------------------------------------------------------+
        // | Get a listing of the scientist if there is a platform name input |
        // +-------------------------------------------------------------------+
        if (!"".equals(platformCode)) {
            ...
            // +------------------------------------------------------------+
            // | get the start and end date for the platform, and get      |
            ...
            try {
                sql =
                    ...
                    " from inventory "+
                    " where planam_code = "+platformCode;
            ...
            try {
                ...
                while (rset.next()) {
                    platformName = checkNull(rset.getString(1));
                    if (dbg) System.out.println("<br> **platformName "+platformName);
            ...
            if ("".equals(startDate)) {
                ...
                if (totalRecords > MAX_RECS) {
                    ...
                    // +------------------------------------------------------------+
                    // | Find out which years have surveys for platform            |
                    // +------------------------------------------------------------+
                    int year1 = 0;                                          //ub01
                    int year2 = 0;                                          //ub01
                    try {
                        ...
                        sql = "select distinct to_char(date_start,'YYYY'), " +//ub01
                              ...
                              "inventory where planam_code = "+platformCode;
                            ...
                            for (int i2 = startIndex; i2 < startIndex + 10; i2++) {
                                ...
                                if (yearArray[i2] != 0) {
                                    ...
                                    String link = new String(
                                        sc.APP + "?" +
                                        sc.SCREEN + "=" + sc.PLATFORMS2 + "&" +
                                        sc.PSC + "=" + sessionCode + "&" +  //ub03
                                        sc.CODE + "=" + platformCode + "&" +
            ...
            if (!"".equals(startDate)) {
                ...
                if (dbg) System.err.println("<br>platformCode = "+platformCode);
                ...
                try {
                    ...
                    sql =
                        " select inv.survey_id, inv.project_name, inv.cruise_name,"+
                        " sc.surname, sc.f_name, sc.title, inst.name,"+
                        " to_char(inv.date_start,'YYYY-MM-DD')," +
                        " to_char(inv.date_end,'YYYY-MM-DD'),"+
                        " inv.data_recorded, st.name, inv.data_available" + //ub03
                        " from inventory inv, planam plan, institutes inst,"+
                        " scientists sc, survey_type st" +
                        " where inv.planam_code = "+platformCode+
                ...
                for (int i = 0; i < numRecs; i++) {
                    ...
                    if (dbg) {
                        ...
                        System.err.println(" institutes    [i] "+institutes      [i]+" i "+ i);
                ...
                if (numRecs == MAX_RECS) {
                   ...
                   if (dbg) System.err.println(" platformCode "+platformCode);

                   //Form moresurvs = new Form("GET", "ListPlatform");
                   Form moresurvs = new Form("GET", sc.APP);
                   moresurvs
                      .addItem(new Hidden(sc.SCREEN, sc.PLATFORMS2))
                      .addItem(new Hidden(sc.CODE, platformCode))
    ...
    public static void main(String args[]) {
        ...
        try {
            HtmlHead hd = new HtmlHead("ListPlatform local");
