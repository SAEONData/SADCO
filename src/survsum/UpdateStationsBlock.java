...
//    String PWD = "ter91qes";
   ...
   //work variables
    ...
    String  pEStationId     = "";
    int     pStnnamCount    = 0;
    int     pPositionCount  = 0;
    String  pDisciplineCode = "";
    String  pDateStart      = "";
    String  pDateEnd        = "";
   ...
   //+------------------------------------------------------------+
    public UpdateStationsBlock(String args[]) {
       ...
       //+------------------------------------------------------------+
        ...
        if (recnum.length() > 0)  {
            iRecnum = Integer.parseInt(recnum);
        } // if (recnum.length() > 0)
       ...
       //+------------------------------------------------------------+
        ...
        rows
            ...
            .addCell(new TableHeaderCell(sc.formatStrBig("Survey Id: <b>" + pSurveyId.toString() + "</b")))
...
//      } // catch (ServerException e)
       ...
       //+------------------------------------------------------------+
        Form updatePSDFrm = new Form("GET", "SurvSum");
...
//         } // catch (ServerException e)
           ...
           //+------------------------------------------------------------+
            if (!prefixErr) pStnidPrefix = pNewPrefixId.substring(0,3);
       ...
       //+------------------------------------------------------------+
        if (pDelUpdated.equals("Y")) {
            ...
            //+------------------------------------------------------------+
            deleteStationRecord(pStationId);
//         try   {
               ...
               //+------------------------------------------------------------+
                if (!boundsErr1 && !boundsErr2)  {
                    if (pRecUpdated.equals("Y")) {
                       ...
                       //+------------------------------------------------------------+
                        editStationRecord(pStationId, pSurveyId, pEStnnam,
                            pEDateStart, pEDateEnd, pELatitude, pELongitude,
                            pEStndep);
//                  try   {
...
//                     } // catch (ServerException e)
                       ...
                       //+------------------------------------------------------------+
                        pEStationId = sc.getNewStationId(pStnidPrefix, pEStationId );
...
//                     } // catch (ServerException e)
                   ...
                   //| flag errors                                                |
                   //+------------------------------------------------------------+
                    if (pStnnamCount == 1) stnnamErr    = true;
                    if (pPositionCount == 1) positionErr  = true;
       ...
       //+------------------------------------------------+
        pDisciplineCode = "1";
        int watSurvConstCount = 0;
        //int watSurvConstCount = count_survey_constituent(pSurveyId,pDisciplineCode);
//        try  {
...
//      int watSurvConstCount = (int)pRecordcount.doubleValue();
       ...
       //+------------------------------------------------+
        pDisciplineCode = "2";
        int sedSurvConstCount = 0;
        //int sedSurvConstCount = count_survey_constituent(pSurveyId,pDisciplineCode);

       //+------------------------------------------------+
       //| Count the number of biolo constituent records  |
       //+------------------------------------------------+
        pDisciplineCode = "3";
        int bioSurvConstCount = 0;
        //int bioSurvConstCount = count_survey_constituent(pSurveyId,pDisciplineCode);
       ...
       //+------------------------------------+
        int stationCount = 0;
        //int stationCount = count_stations(pSurveyId);
//      try  {
...
//      int stationCount = (int)pRecordcount.doubleValue();
       ...
       //+---------------------------------------+
        int prefixCount = 0;
        //int prefixCount = count_stnid_prefix();
//      try  {
...
//      int prefixCount = (int)pRecordcount.doubleValue();
       ...
       //+------------------------------------------------------------+
        Form getWatConstitFrm         = new Form("GET", "SurvSum");
        Form getSedConstitFrm         = new Form("GET", "SurvSum");
        Form getBioConstitFrm         = new Form("GET", "SurvSum");
        Form addWatphyStationsFrm     = new Form("GET", "SurvSum");
        Form addSedphyStationsFrm     = new Form("GET", "xx");
        Form addBiophyStationsFrm     = new Form("GET", "xx");
        Form createWatSpreadSheetFrm  = new Form("GET", "SurvSum");
        ...
        getWatConstitFrm
            .addItem(new Hidden("pscreen", "getSurveyConstituent"))
            .addItem(new Hidden("surveyid", pSurveyId))
        ...
        addWatphyStationsFrm
            .addItem(new Hidden("pscreen", "AddWatphyStations"))
            .addItem(new Hidden("surveyid", pSurveyId))
            .addItem(new Submit("submit", "Water"))
            ;

        createWatSpreadSheetFrm
            .addItem(new Hidden("pscreen", "createWatSpreadSheet"))
            .addItem(new Hidden("surveyid", pSurveyId))
            .addItem(new Hidden("disciplinecode", "1"))
            .addItem(new Submit("submit", "Water"))
            ;
...
//            } // catch (ServerException e)
            ...
            pStnidPrefix = plStationId[0].substring(0,3);
            ...
            for (int i = 0; i < stationCount; i++)  {
                ...
                // +-----------------------------------------------+
                //spldepCount = count_watphy_spldep_station(plStationId[i]);
//            try   {
...
//            spldepCount = (int)pRecordcount.doubleValue();
                ...
                rows
                    ...
                    .addCell(new TableDataCell(sc.formatStr(String.valueOf(plLatitude[i]))))
                    .addCell(new TableDataCell(sc.formatStr(String.valueOf(plLongitude[i]))))
                    ;
                ...
                rows
                    .addCell(new TableDataCell(sc.formatStr(String.valueOf(plStndep[i]))))
                    .addCell(new TableDataCell(sc.formatStr(String.valueOf(plMaxSpldep[i]))))
        ...
        if (pStnidPrefix.length() < 1 && !PSDErr)  {
            ...
            // | Get prefixes         |
            // +----------------------+
            //list_stnid_prefix(plPrefixId, plPrefixName);
//            try  {
...
//            }  //  catch (ServerException e)
            ...
            for (int i = 0; i < prefixCount; i++) {
                ...
                selectPrefixId.addOption(new Option(
                    plPrefixId[i] + " - " + plPrefixName[i],
                    plPrefixId[i],
                    sc.getDefault(plPrefixId[i],pStnidPrefix)
                    ));
            } // for (int i = 0; i < prefixCount; i++)
        ...
        if (pRecUpdated.equals("Y") && recnum.length() > 0)  {
            ...
            latitude    = String.valueOf(plLatitude[iRecnum]);
            longitude   = String.valueOf(plLongitude[iRecnum]);
            stndep      = String.valueOf(plStndep[iRecnum]);
        }  //  if (pRecUpdated.equals("Y"))
        ...
        if (stnidPrefixNeeded) {
            Form selSIPFrm = new Form("GET", "SurvSum");
            selSIPFrm
            ...
            Form newSIPFrm = new Form("GET", "SurvSum");
            newSIPFrm
                .addItem(new Hidden("pscreen", "UpdateStationsBlock"))
                .addItem(new Hidden("surveyid", pSurveyId))
            ...
            if (prefixErr) newSIPFrm.addItem(new SimpleItem(
                "<FONT COLOR=#FF6666><b>Duplicate Prefix ID</b></font><br>"));
            ...
            if ((recnum.length() > 0  && !updateRun) ||
                ...
                Form updateFrm = new Form("GET", "SurvSum");
                updateFrm
                    .addItem(new Hidden("pscreen", "UpdateStationsBlock"))
...
//        int aaa = 0
    ...
    }//constructor UpdateStationsBlock
   ...
   void getPsdData(String pSurveyId) {
     ...
     */
    void addNewPrefix(String pNewPrefixId, String pNewPrefixName) {

        try {

            // get the count
            sql = "select count(*) from stnid_prefix where " +
                  "prefix_id = upper('" + pNewPrefixId + "')";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(sql);
            while (rset.next()) {
                pPrefixCount = (int)rset.getFloat(1);
            } //while (rset.next()) {
            rset.close();
            stmt.close();
            rset = null;
            ...
            if (pPrefixCount == 0) {
                sql = "insert into stnid_prefix values (upper('" +
                      pNewPrefixId + "'),'" + pNewPrefixName + "')";
                stmt = conn.createStatement();
            ...
            this.addItem("<br>Could not process addNewPrefix: " +
                e.getMessage());
            this.addItem("<br>sql = " + sql);
            e.printStackTrace();
        } // try - catch
     ...
     */
    void deleteStationRecord(String pStationId) {

        try {

            sql = "delete from station where station_id = '" + pStationId + "'";
            stmt = conn.createStatement();
            int num = stmt.executeUpdate(sql);
            stmt.close();
            stmt = null;
            ...
            this.addItem("<br>Could not process deleteStationRecord: " +
                e.getMessage());
            this.addItem("<br>sql = " + sql);
            e.printStackTrace();
        } // try - catch
     ...
     *
     */
    void editStationRecord(String pStationId, String pSurveyId, String pEStnnam,
            String pEDateStart, String pEDateEnd, String pELatitude,
            String pELongitude, String pEStndep) {

        try {

            // Check for station name duplication
            sql = "select count(*) from station where " +
                  "survey_id = '" + pSurveyId + "' and " +
                  "station_id  <> '" + pStationId + "' and " +
                  "stnnam = '" + pEStnnam + "'";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(sql);
            while (rset.next()) {
                pStnnamCount = (int)rset.getFloat(1);
            } //while (rset.next()) {
            rset.close();
            stmt.close();
            rset = null;
            ...
            sql = "select count(*) from station where " +
                  "survey_id = '" + pSurveyId + "' and " +
                  "station_id  <> '" + pStationId + "' and " +
                  "latitude = " + pELatitude + " and " +
                  "longitude = " + pELongitude;
            stmt = conn.createStatement();
            rset = stmt.executeQuery(sql);
            while (rset.next()) {
                pPositionCount = (int)rset.getFloat(1);
            } //while (rset.next()) {
            rset.close();
            stmt.close();
            rset = null;
            ...
            if ((pStnnamCount == 0) && (pPositionCount == 0)) {
                sql = "update station set stnnam = '" + pEStnnam + ", " +
                      "latitude = " + pELatitude + ", " +
                      "longitude = " + pELongitude + ", " +
                      //  date_start  = to_date(v_date_start,'YYYY-MM-DD'),
                      //  date_end    = to_date(v_date_end ,'YYYY-MM-DD'),
                      "stndep = " + pEStndep +
                      "where station_id = '" + pStationId + "'";
                stmt = conn.createStatement();
                int num = stmt.executeUpdate(sql);
                stmt.close();
                stmt = null;
...
//    } // spaceIfNull
    ...
    public static void main(String args[]) {
        ...
        HtmlHead hd = new HtmlHead("UpdateStationsBlock local");
        ...
        try {
            ...
            bd.addItem("Error in UpdateStationsBlock: " + e.getMessage());
            e.printStackTrace();
        } // try-catch
