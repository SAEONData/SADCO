...
/*
        ...
        // +------------------------------------------------------------+
        pSurveyId       = getArgument(args,"surveyid");
        pRecUpdated     = getArgument(args,"recUpdated");
        pNewUpdated     = getArgument(args,"newUpdated");
        pEBatchId       = getArgument(args,"batchId");
        pEStartDateTime = getArgument(args,"existStartDateTime");
        pEEndDateTime   = getArgument(args,"existEndDateTime");

//        recordErr       = getArgument(args,"recorderr");
        noEnd           = getArgument(args,"noend");
        aSelRecnum      = getArgument(args,"recnum");
        sDate           = getArgument(args,"sdate");
        sHour           = getArgument(args,"shour");
        sMinute         = getArgument(args,"sminute");
        eDate           = getArgument(args,"edate");
        eHour           = getArgument(args,"ehour");
        eMinute         = getArgument(args,"eminute");
        ...
        rows = new TableRow();
        rows
            .addCell(new TableHeaderCell(formatStrBig("<b>SURVEY SUMMARY REPORT</b>")).setColSpan(80))
            .addCell(new TableHeaderCell(formatStrBig("<b>" + pSurveyId + "</b")))
            ;
...
//        recordErr = "N";
        if (pNewUpdated.equals("Y")) {
            ...
            //System.out.println("<br>---pEStartDatetime--[" + pEStartDateTime);
            //System.out.println("<br>---pEndDatetime--[" + pEEndDateTime);
            addSamplingBatchRecord(pSurveyId, pEBatchId,
                pEStartDateTime, pEEndDateTime);
//         try
//            {
//            sadsql.add_sampling_batch_record(
//               pSurveyId         ,
//               pEBatchId         ,
//               pEStartDateTime   ,
//               pEEndDateTime     ,
...
//            {
//            bd.addItem(new SimpleItem("Could not process add_sampling_batch_record: " + e.getSqlerrm() ));
//            hp.print();
//            return;
//            }  //  catch (ServerException e)
//            if (pBatchCount == 1) recordErr = "Y";
...
//         return;
//         }  //  catch (ServerException e)
//      int batchCount = (int)pRecordcount.doubleValue();
...
//      try
//         {
//         pRecordcount = sadsql.count_survey_dates(pSurveyId);
//         }  //  try
//      catch (ServerException e)
//         {
//         bd.addItem(new SimpleItem("Could not process count_survey_dates: " + e.getSqlerrm() ));
//         hp.print();
//         return;
//         }  //  catch (ServerException e)
//      int datesCount = (int)pRecordcount.doubleValue();
        plDate          = new String[datesCount];

        //System.out.println("<br>----datesCount--[" + datesCount);
        //System.out.println("<br>-----batchCount--[" + batchCount);
...
//      catch (ServerException e)
//         {
//         bd.addItem(new SimpleItem("Could not process list_survey_dates: " + e.getSqlerrm() ));
//         hp.print();
//         return;
//         }  //  catch (ServerException e)
...
//            }  //  catch (ServerException e)
            ...
            rows = new TableRow();
            rows.addCell(new TableDataCell(formatStrBig("<b><u>Sampling Batches</u></b>")).setColSpan(80));
            tab_list.addRow(rows);

            rows = new TableRow();
            rows
                .addCell(new TableDataCell(formatStrBig("Batch Id")))
                .addCell(new TableDataCell(formatStrBig("Start")))
                .addCell(new TableDataCell(formatStrBig("End")))
                .addCell(new TableDataCell(formatStrBig("Hours")))
                ;
            ...
            for (int i = 0; i < batchCount; i++) {
                ...
                rows = new TableRow();
                rows
                    .addCell(new TableDataCell(formatStr("<b><FONT COLOR=#3333FF>" + plBatchId[i] + "</font></b>" )))
                    .addCell(new TableDataCell(formatStr("<b>" + plStartDateTime[i] + GMTstring + "</b>" )))
                    .addCell(new TableDataCell(formatStr("<b>" + plEndDateTime[i] + GMTstring + "</b>" )))
                    ;
                if (plHours[i] <= 0.0) {
                    rows.addCell(new TableDataCell(formatStr("<b><FONT COLOR=#FF0000>" + plHours[i] + " - ERROR</font></b>" )));
                } else {
                    rows.addCell(new TableDataCell(formatStr("<b>" + plHours[i] + "</b>" )));
                }  //  if (plHours <= 0.0)
        ...
        //System.out.println("<br>-----Create date time select lists");
        // +------------------------------------------------------------+
        ...
        for (int batch = 0; batch < 10; batch++) {
            ...
            cBatch++;
        }  //  for (int batch = 65; batch < 76; batch++)


        //System.out.println("<br>-----create default select items");
        // +------------------------------------------------------------+
        // | create default select items                                |
        // +------------------------------------------------------------+
        //System.out.println("<br>--eDate---" + edate);
        if (!eDate.equals(" ")) {
            dsDate   = eDate    ;
            dsHour   = eHour    ;
            dsMinute = eMinute  ;
        ...
        for (int i = 0; i < datesCount; i++) {
            dtItem = plDate[i];
            selectSDate.addOption(new Option(dtItem,dtItem,getDefault(dtItem,dsDate)));
            selectEDate.addOption(new Option(dtItem,dtItem,getDefault(dtItem,deDate)));
        }  //  for (int i = 0; i < datesCount; i++)

        for (int i = 100; i < 124; i++) {
            dtItem = convert.valueOf(i).substring(1,3);
            selectSHour.addOption(new Option(dtItem,dtItem,getDefault(dtItem,dsHour)));
            selectEHour.addOption(new Option(dtItem,dtItem,getDefault(dtItem,deHour)));
        }  //  for (int i = 1990; i < 2011; i++)

        for (int i = 100; i < 160; i++) {
            dtItem = convert.valueOf(i).substring(1,3);
            selectSMinute.addOption(new Option(dtItem,dtItem,getDefault(dtItem,dsMinute)));
            selectEMinute.addOption(new Option(dtItem,dtItem,getDefault(dtItem,deMinute)));
        }  //  for (int i = 1990; i < 2011; i++)


        //System.out.println("<br>--------[Create the form to Add new sampling batch record ");
        // +------------------------------------------------------------+
...
*/
        ...
        try {
            ...
            if (pBatchCount == 0) {
                ...
                sql = "insert into sampling_batch values (" + code + ", '" +
                      pEBatchId + "', '" + pSurveyId + "', "+
                      "to_date('" + pEStartDateTime + "','YYYY-MM-DD HH24 MI') - " +
                          pGMTDiff / 24 + ", " +
                      "to_date('" + pEEndDateTime + "','YYYY-MM-DD HH24 MI') - " +
                          pGMTDiff / 24 + ")";
                stmt = conn.createStatement();
    ...
    void listSamplingBatches(String pSurveyId) {
        ...
        try {
            ...
            while (rset.next()) {
                ...
                plBatchId[i]        = spaceIfNull(rset.getString(j++));
                plStartDateTime[i]  = spaceIfNull(rset.getString(j++));
                plEndDateTime[i]    = spaceIfNull(rset.getString(j++));
    ...
    int countSurveyDates(String pSurveyId) {
        ...
        try {
            ...
            sql = "select date_end - date_start + 1 from inventory " +
                  "where survey_id = '" + pSurveyId + "'";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(sql);
            while (rset.next()) {
